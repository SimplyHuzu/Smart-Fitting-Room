  ...
  try {
    const result = await pool.query(`
      SELECT 
        ...
        EXTRACT(EPOCH FROM (NOW() - r.entry_time))/60 as duration_minutes
      FROM rooms r
...
export const getRoomDetails = async (req: Request, res: Response) => {
  try {
    ...
    const productsResult = await pool.query(`
      ...
      WHERE rp.room_id = $1
      ORDER BY rp.scanned_in_at DESC
    `, [id]);
    ...
    const details: any = {
      ...
      products: productsResult.rows.map(p => ({
        ...
        size: p.size,
        color: p.color,
        scannedIn: p.scanned_in,
        scannedOut: p.scanned_out,
        isMissing: p.is_missing,
      })),
    };

    // Get predicted duration from sessions table (if available)
    try {
      if (room.session_id) {
    ...
    if (prev && prev.status === 'occupied' && status === 'available') {
      (async () => {
        try {
          ...
          // Use session_id from room if available, otherwise generate one
          const sessionId = prev.session_id || crypto.randomUUID?.() || `${Date.now()}-${id}`;
          ...
          // Update session with anomaly results if session exists
          if (prev.session_id) {
            await pool.query(
              `UPDATE sessions 
               SET exit_time = $1, 
                   ...
                   updated_at = NOW()
               WHERE session_id = $6`,
              [now, actualMinutes, detect.is_anomaly, detect.anomaly_score, detect.risk_level, prev.session_id]
            );
          }
...
export const assignRoomWithProducts = async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      // 3. Update room with customer, entry time, and session ID
      await client.query(
        `UPDATE rooms 
         SET status = 'occupied', 
             customer_rfid = $1, 
         ...
         WHERE id = $4
         RETURNING *`,
        [customerCardId, entryTime, sessionId, roomId]
      );
...
export const scanProductOut = async (req: Request, res: Response) => {
  try {
    ...
    const roomResult = await pool.query(
      'SELECT customer_rfid, session_id, entry_time, status FROM rooms WHERE id = $1',
      [id]
    );
    ...
    // Check if product is in room and scanned in for this customer's session
    // If session_id exists, verify the product belongs to this session
    let roomProductQuery = `
      SELECT rp.id 
      FROM room_products rp
      WHERE rp.room_id = $1 
        AND rp.product_id = $2 
        ...
        AND rp.scanned_out_at IS NULL
    `;
    const queryParams: any[] = [id, productId];

    if (room.session_id) {
      roomProductQuery += ` AND rp.session_id = $3`;
      queryParams.push(room.session_id);
    }

    const roomProductResult = await pool.query(roomProductQuery, queryParams);

    if (roomProductResult.rows.length === 0) {
      // Check if product exists but belongs to different session
      const allProductsCheck = await pool.query(
        `SELECT rp.session_id, p.sku, p.name
         FROM room_products rp
         JOIN products p ON rp.product_id = p.id
      ...
      if (allProductsCheck.rows.length > 0) {
        ...
        if (product.session_id && product.session_id !== room.session_id) {
          return res.status(403).json({ 
            error: 'Product belongs to different session',
            message: `This product (${product.sku} - ${product.name}) belongs to a different customer session and cannot be scanned out by the current customer.`
          });
        }
    ...
    if (unscannedItems.rows.length === 0 && room.status === 'occupied') {
      ...
      // Use session_id from room if available, otherwise generate one
      const sessionId = room.session_id || crypto.randomUUID?.() || `${Date.now()}-${id}`;
            ...
            await pool.query(
              `UPDATE sessions 
               SET exit_time = $1, 
                   ...
                   updated_at = NOW()
               WHERE session_id = $6`,
              [now, actualMinutes, detect.is_anomaly, detect.anomaly_score, detect.risk_level, room.session_id]
            );
          }
